...
class ThoughtCrime(APKPlugin):
    ...
    def run(self, module):
        ...
        if (
            not self.zipfile
            or 'res/raw/blfs.key' not in self.zipfile.namelist()
            or 'res/raw/config.cfg' not in self.zipfile.namelist()
        ):
            return None
        key = self.zipfile.open('res/raw/blfs.key').read()
        key = ''.join(['%x' % x for x in key])[:50]
        cipher = Blowfish.new(key, Blowfish.MODE_CBC, "12345678")
